...
function confirmDelete(delUrl) 
  ...
  <tbody>
       <c:forEach var="us" items="${listeruser}"> 
    <tr>
      <th scope="row">${us.prenom}</th>
      <td>${us.nom}</td>
      <td>${us.adresse}</td>
      <td>${us.email}</td>
      <td>${us.telephone}</td>
     
      <td><a href='controleur?action=modifier&id=${us.noUser}' ><img src='pen.png' width='15' /></a></td>
      <td>
      <c:url var="delUrl" value="controleur">
          <c:param name="action" value="supprimer"/> 
          <c:param name="idd" value="${us.noUser}"/> 
      </c:url> 
